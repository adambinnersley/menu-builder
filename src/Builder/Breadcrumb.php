...
class Breadcrumb
{
    ...
    public function createBreadcrumb($class = 'breadcrumb', $itemClass = 'breadcrumb-item', $list = true)
    {
        ...
        if ($this->navArray[0]['uri'] !== $this->links[0]['uri']) {
            $breadcrumb.= (($list === true && $this->isBreadcrumbList() !== false) ? '<li'.(!empty(trim($itemClass)) ? Link::htmlClass($itemClass, '') : '').'>' : '').Link::formatLink($this->navArray[0], '', false, true).($list === true && $this->isBreadcrumbList() ? '</li>' : '');
        }
